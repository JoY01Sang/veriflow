-- Increase committee members from 2 to 5
-- Adds committee_member_3_id, committee_member_4_id, committee_member_5_id

-- =========================================================
-- 1. ADD NEW COMMITTEE MEMBER COLUMNS
-- =========================================================

alter table public.profiles
  add column if not exists committee_member_3_id uuid references public.profiles (id),
  add column if not exists committee_member_4_id uuid references public.profiles (id),
  add column if not exists committee_member_5_id uuid references public.profiles (id);
...
begin
  if (
    ...
    or new.committee_member_3_id is distinct from old.committee_member_3_id
    or new.committee_member_4_id is distinct from old.committee_member_4_id
    or new.committee_member_5_id is distinct from old.committee_member_5_id
...
create or replace function public.assign_thesis_committee(
  ...
  p_committee_member_3_id uuid,
  p_committee_member_4_id uuid,
  p_committee_member_5_id uuid,
...
begin
  ...
  if p_committee_member_3_id is not null and not exists (
    select 1 from public.profiles where id = p_committee_member_3_id and role = 'committee_member'
  ) then
    raise exception 'p_committee_member_3_id must reference a profile with the committee_member role';
  end if;

  if p_committee_member_4_id is not null and not exists (
    select 1 from public.profiles where id = p_committee_member_4_id and role = 'committee_member'
  ) then
    raise exception 'p_committee_member_4_id must reference a profile with the committee_member role';
  end if;

  if p_committee_member_5_id is not null and not exists (
    select 1 from public.profiles where id = p_committee_member_5_id and role = 'committee_member'
  ) then
    raise exception 'p_committee_member_5_id must reference a profile with the committee_member role';
  end if;
  ...
  set
    ...
    committee_member_3_id = p_committee_member_3_id,
    committee_member_4_id = p_committee_member_4_id,
    committee_member_5_id = p_committee_member_5_id,
...
grant execute on function public.assign_thesis_committee(uuid, uuid, uuid, uuid, uuid, uuid, uuid, uuid, uuid) to authenticated;

-- =========================================================
-- 4. UPDATE SUBMIT_DOCUMENT FUNCTION
-- =========================================================
...
begin
  ...
  select array[advisor_id, committee_member_1_id, committee_member_2_id, committee_member_3_id, committee_member_4_id, committee_member_5_id, department_chair_id, graduate_school_id]
    into v_chain
